@doc raw"""
    TreeNeighborhoodSearch{NDIMS}(; search_radius = 0.0, n_points = 0,
...
"""
struct TreeNeighborhoodSearch{NDIMS, US, CL, ELTYPE, PB, UB} <: AbstractNeighborhoodSearch
    cell_list       :: CL
...
function TreeNeighborhoodSearch{NDIMS}(; search_radius = 0.0, n_points = 0,
                                       periodic_box = nothing,
                                       cell_list = DictionaryCellList{NDIMS}(),
    ...
    if search_radius < eps() || isnothing(periodic_box)
        ...
        if any(i -> i < 3, n_cells)
            throw(ArgumentError("the `TreeNeighborhoodSearch` needs at least 3 cells " *
                                "in each dimension when used with periodicity. " *
                                "Please use no NHS for very small problems."))
        end
    end

    return TreeNeighborhoodSearch(cell_list, search_radius, periodic_box, n_cells,
...
See [`TreeNeighborhoodSearch`](@ref) for usage information.
"""
...
function initialize_grid!(neighborhood_search::TreeNeighborhoodSearch, y::AbstractMatrix;
...
function initialize_grid!(neighborhood_search::TreeNeighborhoodSearch{<:Any,
                                                                      ParallelUpdate},
...
function update_grid!(neighborhood_search::Union{TreeNeighborhoodSearch{<:Any,
                                                                        SerialIncrementalUpdate},
                                                 TreeNeighborhoodSearch{<:Any,
                                                                        SemiParallelUpdate}},
                      y::AbstractMatrix;
                      parallelization_backend = default_backend(y),
                      eachindex_y = axes(y, 2))
...
@inline function mark_changed_cells!(neighborhood_search::TreeNeighborhoodSearch{<:Any,
                                                                                 SemiParallelUpdate},
...
function update_grid!(neighborhood_search::TreeNeighborhoodSearch{<:Any,
                                                                  ParallelIncrementalUpdate},
                      y::AbstractMatrix; parallelization_backend = default_backend(y),
                      eachindex_y = axes(y, 2))
    (; cell_list, update_buffer) = neighborhood_search
...
function copy_neighborhood_search(nhs::TreeNeighborhoodSearch, search_radius, n_points;
    ...
    return TreeNeighborhoodSearch{ndims(nhs)}(; search_radius, n_points, periodic_box,
                                              cell_list,
